			    ...
			    int max_buffer_len, int* buffer_len, int* ifail)
{
    ...
    if(header || file_ptr->format == FFTEXT)
    {
        ...
        else
        {
            ...
            *buffer_len = strlen(input_output_buffer);
			...
			/* Turn off this warning individually as we cannot use a secure string function
    ...
    {
        int chars = fread(buffer, (unsigned)(sizeof(char)), max_buffer_len, file_ptr->stream);
        if(chars == 0)
        {
			*ifail = (feof(file_ptr->stream) ? FR_end_of_file : FR_read_fail);
			return;
